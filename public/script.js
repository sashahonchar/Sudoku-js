	...
	// Loading
		...
		P.serializeState = function() {
			return {
				...
				cells: this.puzzle.cells.map(cell => cell.toJSON()).filter(json => Object.keys(json).length > 0)
			};
		};
		P.deserializeState = function(state) {
			...
			return this.loadRemoteCTCPuzzle(state.puzzleId)
				.then(() => state.cells.forEach(cellState => {
					var [r, c] = Puzzle.parseRCVal(cellState.rc);
...
const App = (() => {
	...
	// Puzzle
		...
		P.loadCTCPuzzle = function(ctcPuzzle) {
			var puzzle = this.convertPuzzle(ctcPuzzle);
